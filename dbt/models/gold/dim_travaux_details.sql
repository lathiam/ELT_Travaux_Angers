{{ config(
  materialized='table',
  partition_by = {"field": "mois_debut", "data_type": "date"},
  cluster_by   = ["statut", "type_travaux", "annee_debut"],
  on_schema_change = "sync_all_columns"
) }}

{# -----------------------------------------------------------
   DIM "travaux" (toutes années) depuis le STAGING
   - pas de filtre d’exclusion
   - enrichissements métier + indicateurs de suivi
   ----------------------------------------------------------- #}
